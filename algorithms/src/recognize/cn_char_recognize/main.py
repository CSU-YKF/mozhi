...
def parse_args():
    ...
    parser.add_argument('--train_dir', type=str, default='../data/HWDB/Train', help='Directory containing the training .gnt files')
    parser.add_argument('--test_dir', type=str, default='../data/HWDB/Test', help='Directory containing the test .gnt files')
    parser.add_argument('--mapping_file', type=str, default='../data/HWDB/mapping.pkl', help='Path to the mapping file')
    parser.add_argument('--num_epochs', type=int, default=14, help='Number of epochs to train')
...
def main():
    ...
    writer = tensorboard.SummaryWriter(log_dir=f'runs/HWDB_experiment/{current_time}')
    ...
    train_loader = data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=3)
    ...
    # model = torchvision.saved_models.efficientnet.efficientnet_v2_l(weights=None, num_classes=num_classes)
    ...
    # scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda e: 0.1 ** (e // 30))
    num_batches = len(train_loader)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=num_batches*2,
                                                                     last_epoch=-1, verbose=True, T_mult=2)
    # Training loop
