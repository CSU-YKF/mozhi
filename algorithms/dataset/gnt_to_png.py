...
data_dir = 'png_data'
# 路径为存放数据集解压后的.gnt文件
train_data_dir = os.path.join('', 'Gnt1.0Train')
test_data_dir = os.path.join('', 'Gnt1.0Test')
...
# 路径为data文件夹下的子文件夹，train为存放训练集.png的文件夹
    dir_name = 'png_data/train/' + '%0.5d' % char_dict[tagcode_unicode]
    print(dir_name)
...
# 路径为data文件夹下的子文件夹，test为存放测试集.png的文件夹
    dir_name = 'png_data/test/' + '%0.5d' % char_dict[tagcode_unicode]
    if not os.path.exists(dir_name):
        os.mkdir(dir_name)
