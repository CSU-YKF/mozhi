...
这是一个简洁且易于修改的 ImageNet 训练脚本，可利用一些最新的网络和训练技术重现 ImageNet训练结果的脚本。
它倾向于典型的 PyTorch和标准 Python 风格，而不是试图"无所不能"。
尽管如此，它在速度和训练结果的改进。
你可以根据自己的需要重新使用。

该脚本由 PyTorch ImageNet 示例的早期版本启动(https://github.com/pytorch/examples/tree/master/imagenet)

从英伟达 Apex 示例中采用的英伟达 CUDA 特定加速(https://github.com/NVIDIA/apex/tree/master/examples/imagenet)
...
# 第一个 arg 解析器只解析 --config 参数，该参数用于
# 加载包含关键值的 yaml 文件，这些关键值会覆盖下面主解析器的默认值
...
parser.add_argument('--model', default='resnet101', type=str, metavar='MODEL',
                    help='要训练的模型名称（默认："countception）')
parser.add_argument('--pretrained', action='store_true', default=False,
                    help='从指定网络的预训练版本开始（如果有的话）')
parser.add_argument('--initial-checkpoint', default='', type=str, metavar='PATH',
                    help='从该检查点初始化模型（默认值：无）')
parser.add_argument('--resume', default='', type=str, metavar='PATH',
                    help='从检查点恢复全部模型和优化器状态（默认值：无）')
parser.add_argument('--no-resume-opt', action='store_true', default=False,
                    help='防止在恢复模型时恢复优化器状态')
parser.add_argument('--num-classes', type=int, default=3, metavar='N',
                    help='标签类别数（默认：1000）')
parser.add_argument('--gp', default=None, type=str, metavar='POOL',
                    help='全局数据池类型（fast、avg、max、avgmax、avgmaxc）之一。模型默认为 "无"。')
parser.add_argument('--img-size', type=int, default=None, metavar='N',
                    help='Image patch size (default: None => model default)')
parser.add_argument('--crop-pct', default=None, type=float,
                    metavar='N', help='输入图像中心裁剪百分比（仅用于验证）')
parser.add_argument('--mean', type=float, nargs='+', default=[0.662, 0.662, 0.662], metavar='MEAN',
                    help='覆盖数据集的平均像素值')
parser.add_argument('--std', type=float, nargs='+', default=[0.2721, 0.2721, 0.2721], metavar='STD',
                    help='覆盖数据集的标准偏差')
parser.add_argument('--interpolation', default='', type=str, metavar='NAME',
                    help='图像大小调整插值类型（覆盖模型）')
parser.add_argument('-b', '--batch-size', type=int, default=32, metavar='N',
                    help='训练的输入批次大小（默认值：32）')
parser.add_argument('-vb', '--validation-batch-size-multiplier', type=int, default=1, metavar='N',
                    help='验证批量与训练批量之比（默认：1）')
...
# 批量规范参数（目前仅适用于基于 gen_efficientnet 的模型）
parser.add_argument('--bn-tf', action='store_true', default=False,
                    help='Use Tensorflow BatchNorm defaults for saved_models that support it (default: False)')
...
parser.add_argument('--log-interval', type=int, default=8, metavar='N',
                    help='在记录训练状态之前要等待多少批次')
parser.add_argument('--recovery-interval', type=int, default=8, metavar='N',
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if args.local_rank == 0 and (last_batch or batch_idx % args.log_interval == 0):
                ...
                _logger.info(
                    ...
                    'Acc@2: {top5.val:>7.4f} ({top5.avg:>7.4f})'.format(
